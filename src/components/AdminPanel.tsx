      ...
      >
        ...
        <div className="flex border-b border-gray-100 bg-gray-50/50">
          <button 
            ...
            className={`flex-1 py-4 text-sm font-bold transition-all border-b-2 ${activeTab === 'users' ? 'border-red-500 text-red-600 bg-white' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
          >
            إدارة المستخدمين
          </button>
          <button 
            onClick={() => setActiveTab('generate')}
            className={`flex-1 py-4 text-sm font-bold transition-all border-b-2 ${activeTab === 'generate' ? 'border-red-500 text-red-600 bg-white' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
          >
            توليد حسابات خاصة
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
          {activeTab === 'users' ? (
            ...
            <motion.div
              ...
              className="space-y-6"
            >
              <div className="bg-red-50 p-6 rounded-[2rem] border border-red-100 text-right">
                <h3 className="text-lg font-black text-red-700 mb-2">توليد حساب جديد</h3>
                <p className="text-sm text-red-600 opacity-80">يمكنك هنا إنشاء حسابات خاصة ببيانات دخول محددة للأشخاص الذين لا يملكون حساب جوجل.</p>
              </div>

              <form onSubmit={handleGenerateAccount} className="space-y-4">
                {genError && (
                  <div className="p-4 bg-red-100 text-red-600 rounded-2xl text-sm font-bold text-center">
                    {genError}
                  </div>
                )}
                {genSuccess && (
                  <div className="p-4 bg-green-100 text-green-600 rounded-2xl text-sm font-bold text-center">
                    تم إنشاء الحساب بنجاح!
                  </div>
                )}

                <div className="space-y-2">
                  <label className="block text-sm font-bold text-gray-700 text-right pr-2">الاسم المستعار</label>
                  <input
                    type="text"
                    placeholder="مثال: أحمد محمد"
                    className="w-full p-4 bg-gray-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-red-500 text-right font-bold"
                ...
                <div className="space-y-2">
                  <label className="block text-sm font-bold text-gray-700 text-right pr-2">اسم المستخدم (Username)</label>
                  <input
                    type="text"
                ...
                <div className="space-y-2">
                  <label className="block text-sm font-bold text-gray-700 text-right pr-2">كلمة المرور</label>
                  <input
                    type="text"
                ...
                <button
                  type="submit"
                  disabled={actionLoading}
                  className="w-full py-4 bg-red-500 text-white rounded-2xl font-black hover:bg-red-600 transition-all shadow-lg disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {actionLoading ? (
